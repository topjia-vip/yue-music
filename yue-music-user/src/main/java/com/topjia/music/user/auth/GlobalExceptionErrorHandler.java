    ...
    private ErrorBody getErrorBody(String status) {
        ...
        if (String.valueOf(ResultEnum.UPLOAD_IMAGE_ERROR.getCode()).equals(status)) {
            ...
            errorBody = ErrorBody.builder()
                    ...
                    .status(HttpStatus.OK)
                    .build();
        } else if (String.valueOf(ResultEnum.EMIL_ERROR.getCode()).equals(status)) {
            // 邮箱验证错误
            errorBody = ErrorBody.builder()
                    .code(500)
                    .data(ResultDTO.builder()
                            .status(ResultEnum.EMIL_ERROR.getCode())
                            .message(ResultEnum.EMIL_ERROR.getDesc())
                            .build())
                    .status(HttpStatus.OK)
                    .build();
            ...
            errorBody = ErrorBody.builder()
                    ...
                    .status(HttpStatus.OK)
                    .build();
        } else if (String.valueOf(ResultEnum.CAPTCHA_EMPTY.getCode()).equals(status)) {
            // 验证码未获取
            errorBody = ErrorBody.builder()
                    .code(500)
                    .data(ResultDTO.builder()
                            .status(ResultEnum.CAPTCHA_EMPTY.getCode())
                            .message(ResultEnum.CAPTCHA_EMPTY.getDesc())
                            .build())
                    .status(HttpStatus.OK)
                    .build();
        } else if (String.valueOf(ResultEnum.CAPTCHA_UNEQUAL.getCode()).equals(status)) {
            // 验证码不一致
            errorBody = ErrorBody.builder()
                    .code(500)
                    .data(ResultDTO.builder()
                            .status(ResultEnum.CAPTCHA_UNEQUAL.getCode())
                            .message(ResultEnum.CAPTCHA_UNEQUAL.getDesc())
                            .build())
                    .status(HttpStatus.OK)
                    .build();
        }
